...
def select_input_device(preferred_device: Optional[Any] = None) -> Tuple[Optional[int], str]:
    ...
    try:
        ...
        print(f"[ASR] Unable to list audio devices: {exc}", file=sys.stderr)
        return None, ""
...
class SpeechRecognizer:
    ...
    def __init__(
        ...
        print(f"[ASR] Input device selected: {self.input_device_name or 'default'}", file=sys.stderr)
    ...
    def listen_and_transcribe(
        ...
        """
        print("[ASR] Listening started", file=sys.stderr)
        ...
        with sd.RawInputStream(
            ...
            if stop_event is not None and stop_event.is_set():
                print("[VOSK] Cancelled before warmup", file=sys.stderr)
                return None
            ...
            while True:
                if stop_event is not None and stop_event.is_set():
                    print("[VOSK] Cancelled", file=sys.stderr)
                    break

                if time.time() - start_time > timeout:
                    print("[VOSK] Listening timeout", file=sys.stderr)
                    break
                ...
                try:
                    ...
                    if stop_event is not None and stop_event.is_set():
                        print("[VOSK] Cancelled while waiting for audio", file=sys.stderr)
                        break
                    continue
                ...
                if self.recognizer.AcceptWaveform(data):
                    ...
                    if len(text) < 2:
                        print("[VOSK] Ignoring empty result", file=sys.stderr)
                        continue
    ...
    def listen_and_transcribe(self, timeout=15):
        ...
        print("Texto detectado:", result, file=sys.stderr)
        return result
    """
    """
